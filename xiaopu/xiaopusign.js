...
2021,01,16号增加金蛋奖励 防黑号增加直播奖励在0点到7点不会执行 直播在8点左右才会有直播

=========================================


一共2个cookie 一共2个Secrets 多账号换行
...
const Body =  $.getval('xiaopuBody');//1,获取看视频,2,获取金蛋
    ...
    setTimeout( ()=>{
      ...
      $.post(Url, async (err, resp, data) => {
        try {
          ...
          $.complete = data;
if($.complete.resultCode== 1)
dd+="看视频金蛋  +"+$.complete.data.goldCoinNumber+"金币\n";

if($.complete.resultCode== 0)
dd+="看视频金蛋 "+$.complete.errorDesc+'\n';
...
if($request.url.indexOf("give_gold_coin_by_video") > 0&&$request.body.indexOf('"type":1') > 0){
        if(Body==1){
...
if(Body==2){
if($request.body.indexOf('"type":2') > 0){
    
const coinbody = $request.body;
if(coinbody)
    $.setdata(coinbody,"coinbody" + $.idx);
 $.msg($.name + $.idx,"","[获取金蛋数据]✅成功");}
